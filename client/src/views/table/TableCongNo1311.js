...
function TableCongNo1311() {
  const { promiseInProgress } = usePromiseTracker({
    area: areas.tableOwe,
    delay: 0,
  });

  const { setTable1311 } = useStoreActions((actions) => actions);
  const { table1311 } = useStoreState((state) => state);
  ...
  useEffect(() => {
    if (dateRange[0] && dateRange[1]) {
      getDataTable();
      setTable1311({ ...table1311, dateRange: dateRange });
    }
  }, [dateRange]);
  ...
  return (
    <CCard>
      <CCardHeader className="inputHeader">
        <h3 className="mb-3">Công Nợ Phải Thu Khách Hàng</h3>
        <CRow>
        ...
        {promiseInProgress ? null : (
          <CDataTable
            ...
            onRowClick={(item) => {
              ...
              history.push("/tables/debt-fact/detail");
            }}
          />
        )}
      </CCardBody>
...
export default TableCongNo1311;
